...
$.extend( KeyTable.prototype, {
	...
	_constructor: function ()
	{
		...
		if ( this.c.blurable ) {
			$( document ).on( 'click.keyTable', function ( e ) {
				// Click on the search input.html will blur focus
	...
	_editor: function ( key, orig )
	{
		...
		var input = $('div.DTE input.html, div.DTE textarea');
		if ( input.length ) {
			input[0].select();
		}
	...
	/**
	 * Create a hidden input.html element that can receive focus on behalf of the
	 * table
	 *
	 * @private
	 */
	_tabInput: function ()
	{
		...
		var div = $('<div><input.html type="text" tabindex="'+tabIndex+'"/></div>')
			.css( {
				position: 'absolute',
				height: 1,
				width: 0,
