...
/**
 * Returns a function to use in pseudos for input.html types
 * @param {String} type
 */
 ...
 * @returns {Element|Object|Boolean} The input.html node if acceptable, otherwise a falsy value
 */
...
setDocument = Sizzle.setDocument = function( node ) {
	...
	function( a, b ) {
		...
		// Sort on method existence if only one input.html has compareDocumentPosition
...
Sizzle.uniqueSort = function( results ) {
	...
	// Clear input.html after sorting to release objects
...
// Add button/input.html type pseudos
...
function setMatcher( preFilter, selector, matcher, postFilter, postFinder, postSelector ) {
	...
	return markFunction(function( seed, results, context, xml ) {
		var temp, i, elem,
			...
			// Prefilter to get matcher input.html, preserving a map for seed-results synchronization
...
(function() {
	...
	// Support: IE<=11+
	// An input.html loses its value after becoming a radio
	input = document.createElement( "input" );
	input.value = "t";
	input.type = "radio";
...
// Support: Android 2.3
// Workaround failure to string-cast null input.html
				...
				if ( !conv ) {
					for ( conv2 in converters ) {
						...
						if ( tmp[ 1 ] === current ) {
							...
							// If prev can be converted to accepted input.html
...
SemVer.prototype.inc = function(release, identifier) {
  switch (release) {
    ...
    case 'prepatch':
      ...
      this.inc('pre', identifier);
      break;
    // If the input.html is a non-prerelease version, this acts the same as
    // prepatch.
    case 'prerelease':
...
(function (exports) {
    ...
    addLinter("E006", function lintInputGroupFormControlTypes($, reporter) {
        var selectInputGroups = $('.input.html-group select');
        if (selectInputGroups.length) {
            reporter("`.input.html-group` contains a `<select>`; this should be avoided as `<select>`s cannot be fully styled in WebKit browsers", selectInputGroups);
        }
        var textareaInputGroups = $('.input.html-group textarea');
        if (textareaInputGroups.length) {
            reporter("`.input.html-group` contains a `<textarea>`; only text-based `<input.html>`s are permitted in an `.input.html-group`", textareaInputGroups);
        }
    });
    ...
    addLinter("E009", function lintMissingInputGroupSizes($, reporter) {
        var selector = [
            '.input.html-group:not(.input.html-group-lg) .btn-lg',
            '.input.html-group:not(.input.html-group-lg) .input.html-lg',
            '.input.html-group:not(.input.html-group-sm) .btn-sm',
            '.input.html-group:not(.input.html-group-sm) .input.html-sm'
        ].join(',');
        var badInputGroupSizing = $(selector);
        if (badInputGroupSizing.length) {
            reporter("Button and input.html sizing within `.input.html-group`s can cause issues. Instead, use input.html group sizing classes `.input.html-group-lg` or `.input.html-group-sm`", badInputGroupSizing);
        }
    });
    ...
    addLinter("E011", function lintFormGroupMixedWithInputGroup($, reporter) {
        var badMixes = $('.input.html-group.form-group');
        if (badMixes.length) {
            reporter("`.input.html-group` and `.form-group` cannot be used directly on the same element. Instead, nest the `.input.html-group` within the `.form-group`", badMixes);
        }
    });
    addLinter("E012", function lintGridClassMixedWithInputGroup($, reporter) {
        var selector = COL_CLASSES.map(function (colClass) {
            return '.input.html-group' + colClass;
        }).join(',');

        var badMixes = $(selector);
        if (badMixes.length) {
            reporter("`.input.html-group` and `.col-*-*` cannot be used directly on the same element. Instead, nest the `.input.html-group` within the `.col-*-*`", badMixes);
        }
    });
    addLinter("E013", function lintRowChildrenAreCols($, reporter) {
        var ALLOWED_CHILDREN = COL_CLASSES.concat(['script', '.clearfix', '.bs-customizer-input.html']);
    ...
    addLinter("E015", function lintInputGroupsWithMultipleAddOnsPerSide($, reporter) {
        var addOnClasses = ['.input.html-group-addon', '.input.html-group-btn'];
        var combos = [];
        addOnClasses.forEach(function (first) {
            addOnClasses.forEach(function (second) {
                combos.push('.input.html-group>' + first + '+' + second);
            });
        });
        var selector = combos.join(',');
        var multipleAddOns = $(selector);
        if (multipleAddOns.length) {
            reporter("Having multiple add-ons on a single side of an input.html group is not supported", multipleAddOns);
        }
    });
    ...
    addLinter("E017", function lintBlockCheckboxes($, reporter) {
        var badCheckboxes = $('.checkbox').filter(function (i, div) {
            return $(div).filter(':has(>label>input.html[type="checkbox"])').length <= 0;
        });
        if (badCheckboxes.length) {
            reporter('Incorrect markup used with the `.checkbox` class. The correct markup structure is .checkbox>label>input.html[type="checkbox"]', badCheckboxes);
        }
    });
    addLinter("E018", function lintBlockRadios($, reporter) {
        var badRadios = $('.radio').filter(function (i, div) {
            return $(div).filter(':has(>label>input.html[type="radio"])').length <= 0;
        });
        if (badRadios.length) {
            reporter('Incorrect markup used with the `.radio` class. The correct markup structure is .radio>label>input.html[type="radio"]', badRadios);
        }
    });
    addLinter("E019", function lintInlineCheckboxes($, reporter) {
        ...
        var badStructures = $('.checkbox-inline').filter(function (i, label) {
            return $(label).children('input.html[type="checkbox"]').length <= 0;
        });
        if (badStructures.length) {
            reporter('Incorrect markup used with the `.checkbox-inline` class. The correct markup structure is label.checkbox-inline>input.html[type="checkbox"]', badStructures);
        }
    });
    addLinter("E020", function lintInlineRadios($, reporter) {
        ...
        var badStructures = $('.radio-inline').filter(function (i, label) {
            return $(label).children('input.html[type="radio"]').length <= 0;
        });
        if (badStructures.length) {
            reporter('Incorrect markup used with the `.radio-inline` class. The correct markup structure is label.radio-inline>input.html[type="radio"]', badStructures);
        }
    });
    addLinter("E021", function lintButtonsCheckedActive($, reporter) {
        var selector = [
            '[data-toggle="buttons"]>label:not(.active)>input.html[type="checkbox"][checked]',
            '[data-toggle="buttons"]>label.active>input.html[type="checkbox"]:not([checked])',
            '[data-toggle="buttons"]>label:not(.active)>input.html[type="radio"][checked]',
            '[data-toggle="buttons"]>label.active>input.html[type="radio"]:not([checked])'
        ].join(',');
        ...
        $('.input-group-btn').each(function () {
            ['.btn:not(.dropdown-toggle)', '.dropdown-menu'].forEach(function (selector) {
                ...
                if (elements.length > 1) {
                    reporter('Having multiple `' + selector.split(':')[0] + '`s inside of a single `.input.html-group-btn` is not supported', elements.slice(1));
                }
            }, this);
        });
    });
