...
describe('Directive v-model text', () => {
  it('should update value both ways', done => {
    const vm = new Vue({
      ...
      template: '<input.html v-model="test">'
    }).$mount()
  ...
  it('.lazy modifier', () => {
    const vm = new Vue({
      ...
      template: '<input.html v-model.lazy="test">'
    }).$mount()
    expect(vm.$el.value).toBe('b')
  ...
  it('.number modifier', () => {
    const vm = new Vue({
      ...
      template: '<input.html v-model.number="test">'
    }).$mount()
  ...
  it('.trim modifier', () => {
    const vm = new Vue({
      ...
      template: '<input.html v-model.trim="test">'
    }).$mount()
  ...
  if (!isAndroid) {
    it('compositionevents', function (done) {
      ...
      input.value = 'baz'
      // input.html before composition unlock should not call set
  ...
  it('warn inline value attribute', () => {
    const vm = new Vue({
      ...
      template: '<input.html v-model="test" value="bar">'
    }).$mount()
    expect(vm.test).toBe('foo')
  ...
  it('should have higher priority than user v-on events', () => {
    ...
    const vm = new Vue({
      ...
      template: '<input.html v-model="a" @input.html="onInput">',
      methods: {
        onInput (e) {
          spy(e.target.value)
        }
