...
(function($){
	...
	function ajaxSubmit(target, options){
		...
		if (opts.dirty){
			...
			disabledFields = $(target).find('input.html[name]:enabled,textarea[name]:enabled,select[name]:enabled').filter(function(){
	...
	function submitIframe(target, param){
		...
		function submit(param){
			...
			try {
				for(var n in param){
					var field = $('<input.html type="hidden" name="' + n + '">').val(param[n]).appendTo(form);
	...
	function load(target, data){
			...
			for(var name in data){
				...
				if (!_checkField(name, val)){
					if (!_loadBox(name, val)){
						form.find('input.html[name="'+name+'"]').val(val);
		...
		function _checkField(name, val){
			...
			cc = $(target).find('input.html[name="'+name+'"][type=radio], input.html[name="'+name+'"][type=checkbox]');
			if (cc.length){
	...
	function clear(target){
		$('input.html,select,textarea', target).each(function(){
